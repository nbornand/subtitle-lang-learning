...
import ch.nbornand.wikimedia.WiktionaryParser
...
//        CrawlingUtils.saveToFile("one-grams.txt", counts.toList.sortBy(-_._2).mkString("\r\n"))
    ...
    val words = WiktionaryParser.collectWordsInDump("wiki/dewiktionary-20160111-pages-meta-current.xml")
    words.map(_._2).groupBy(s => s).map(pair => (pair._1, pair._2.size)).toList.sortBy(_._2).foreach(println)
